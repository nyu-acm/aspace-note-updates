...
#This script modifies or deletes a specific note type in
#ArchivesSpace based on information supplied by the user.
#The user can select whether their input must simply be contained in
#the original note text, or if it must exactly match the original note content.
#String matches to user input are case sensitive.
#This script is adapted from the advancedNoteEdit.py script created
#by the Rockefeller Archives Center.
#We borrow their original code with much gratitude.
...
# Replaces General notes that includes input notecontent
    notes = ao["notes"]
    for index, n in enumerate(notes):
        try:
            if n["type"] == notetype:
...
# Replaces General notes that match input notecontent exactly
    notes = ao["notes"]
    for index, n in enumerate(notes):
        try:
            if n["type"] == notetype:
